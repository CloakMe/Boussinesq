function [tt, max_v, t, v1u, v2u]=BE2D_v6_0ld(x,y,h,tau,t_end,beta1,al,estep,u_t0,dudt_t0)
beta2 = 1;
...
sx = length(x);
   ...
   %}
    while(t(k)<t_end-1.0e-08)
        ...
        d4vz = diff2y(  (-beta2/h^2) * d2y_gmI(v2z,gm,vdah) + (beta1/tau^2)*(-2*v2mo + v2mt) , vdah  );
        %b=b(v1z,v1mo,v2z,v2mo,v2mt)
       ...
       % d2vz = al*diff2y((v2z.^2)  , vdah );
        ...
        d3vz = diff2x( ( -beta2/h^2)*d2x_gmI(v1u ,gm , vdah) -( beta2/h^2)* diff2y(v2z+v1u , vdah)   - beta1*b2   , vdah );
        ...
        if(mod(k,estep)==0)
            ...
            str = ['SOL\MM_' num2str(e) '.mat'];
            save (str,  'v1u');%(1:xstep:end,1:xstep:end);
            str = ['SOL\MM2_' num2str(e) '.mat'];
   ...
   clear('v1mo');  clear('v2mo'); 
    ...
    function d2y=diff2y(M,vdah)
      vdah(:,1:end-1) = M(:,2:end);    
      vdah(:,2:end) = vdah(:,2:end) + M(:,1:end-1);
      d2y = vdah - 2*M;
      %d2y = vdah/h^2;
      
function d2x=diff2x(M,vdah)
      vdah(1:end-1,:) = M(2:end,:); 
      vdah(2:end,:) = vdah(2:end,:) + M(1:end-1,:);
      d2x = vdah - 2*M;
      %d2x = vdah/h^2;
      
function resm=dh_gmI(M,gm,vdah,vdah2)
      vdah(1:end-1,:) = M(2:end,:); 
      vdah(2:end,:) = vdah(2:end,:) + M(1:end-1,:);
