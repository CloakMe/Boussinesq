function [tt, max_v, t, vu]=BE2D_v3_sit(x,y,h,tau,t_end,beta1,beta2,al,estep,u_t0,dudt_t0)
...
max_v(1) = max(max(u_t0));
   ...
   figure(2)
    ...
    while(t(k)<t_end)
        ...
        [vu,cnt] = SIT_v3_2d(W,IDH,s_idh,vz,vmo,h,tau,sx,al,beta1,beta2,vdah,vdah);
        if(cnt ~= cnt_old)
            iteration_count = cnt
        end
