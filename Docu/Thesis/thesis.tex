...
\section{Закони за запазване при непрекъснатата задача}
В пространството от функции, които са нула по границата $\partial\Omega$
$$ \{ f_b : \Omega \rightarrow R | f_b(x) = 0, x \in \partial\Omega\},$$
дефинираме оператора $A$, така че да удовлетворява $Av=-\Delta_h v=-v_{\bar{x}x} - v_{\bar{y}y}$, където
$$v_{\bar{x}x} = \frac{1}{h^2}(x_{i+1} - 2x_{i} + x_{i-1}), i = 1,\cdots, N_x-1 $$ 
$$ v_{x} =  \frac{1}{h}(x_{i+1} - x_{i}), i = 0,\cdots, N_x-1$$
$$ v_{\bar{x}} =  \frac{1}{h}(x_{i} - x_{i-1}), i = 1,\cdots, N_x.$$

Аналогични са дефинициите при $y$ и $t$. А сега да разгледаме диференчната схема
...
\begin{align*}
&\left( \beta (E+A^{-1})- \frac{\tau^2}{2}(\beta E+A ) \right) \left( (v^{k+1})^2 - 2v^{k+1}v^{k} + 2v^{k}v^{k-1} - (v^{k-1})^2   \right )  + \\
& +\frac{1}{2} (\beta E +A ) \left( (v^{k+1})^2 - (v^{k-1})^2 \right ) - \\
&- \frac{\alpha \beta}{3}\left( (v^{k+1})^3-(v^{k-1})^3 \right) =0
\end{align*}
след което част от първият член е пренесена във вторият с обща част $(\beta E + A )$
\begin{align*}
&\left( \beta (E+A^{-1})- \frac{\tau^2}{4}(\beta E+A ) \right) \left( (v^{k+1})^2 - 2v^{k+1}v^{k} + 2v^{k}v^{k-1} - (v^{k-1})^2   \right )  + \\
& +\frac{1}{4} (\beta E +A ) \left( (v^{k+1})^2 + 2v^{k+1}v^{k} -2v^{k}v^{k-1} -  (v^{k-1})^2  \right ) - \\
&- \frac{\alpha \beta}{3}\left( (v^{k+1})^3-(v^{k-1})^3 \right) =0.
\end{align*}
Добавя се $(v^{k})^2 - (v^{k})^2 = 0$ в десните множители на първия и втория член и $(v^{k})^3 - (v^{k})^3 = 0$ в третия, за да се получи:
 \begin{align*}
&\left( \beta (E+A^{-1})- \frac{\tau^2}{4}(\beta E+A ) \right) \left( (v^{k+1} - v^{k} )^2 - (v^{k} - v^{k-1})^2   \right )  + \\
& +\frac{1}{4} (\beta E +A ) \left( (v^{k+1}+v^{k})^2 -  (v^{k}+v^{k-1})^2  \right ) - \\
