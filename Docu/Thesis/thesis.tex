...
$$
където с $\tau = T/N_t$ сме означили стъпката по времето. Стойността на неизвестната функция $u$ в точка от мрежата $(x_i,y_j,t_k)$ е означена с $u_{i,j}^{(k)}$,т.е. долния индекс се използва за пространствената дискретизация, а горния за времева дискретизация. За дискретизацията на оператора на Лаплас са използвани централни крайни разлики с различна степен на апроксимация:
...
\ee
Последното уравнение може да се запише в матричен вид отчитайки всички вътрешни точки от мрежата. За целта се дефинира следната помощна редица от матрици $\left(V^{(k)}\right) := (u_{i,j}^{(k)})$ при $k=0,..,N_t$. Колоните в тази матрица изобразяват неизвестната функция по $x$-оста, а редовете съответно неизвестната функция по $y$-оста. Така за дискретния оператор на Лаплас $\Delta_h$ се получава, че: 
\be\label{DeltaH}
...
\end{bmatrix},
\]
защото $u \in J$ и $u(x_0, y_j, t_k) = u(x_{N_x}, y_j, t_k) = 0$. По същия начин, но с различна големина, изглежда и матрицата $\Delta_{h,p,y}$, защото $u \in J$ и $u(x_i, y_0, t_k) = u(x_i, y_{N_y}, t_k) = 0$.  При $p=4$ имаме, че 
\[
...
\end{bmatrix}
\]
Вторият и предпоследният редове на матрицата $\Delta_{h,4,x}$ се обясняват с условието, че $u \in J$ и съответно $u(x_0, y_j, t_k) = u(x_{N_x}, y_j, t_k) = 0$ както е при $O(h^2)$ апроксимацията. 
...
\end{bmatrix}
\]
Третият и предпредпоследният ред на матрицата $\Delta_{h,6,x}$ се обясняват с условието, че $u \in J$ и съответно $u(x_0, y_j, t_k) = u(x_{N_x}, y_j, t_k) = 0$ както е при $O(h^2)$ апроксимацията. 
...
\ee
където $V_h, F_h \in \RR^{(N_y-2)\times(N_x-2)}$ са матрици с елементи $v(x_i,y_j)$ и съответно $f(x_i,y_j)$. Нека за матриците $\Delta_{h,p,x}^T$ и $\Delta_{h,p,y}^T$, както са дефинирани в ``Нулево гранично условие'', имаме следните собствени стойности $\lambda_{x,i}$, $\lambda_{y,j}$ и собствени вектори $s_{x,i}$, $s_{y,j}$:
\begin{align}
...
\be\label{subst}
X_h := ( S_x^T * V_h * S_y ) \in \RR^{(N_y-2)\times(N_x-2)},
\ee
...
&E_h(v^{(k)})=\nonumber\\
&\left( \left( \beta (E+A_h^{-1})- \frac{\tau^2}{4}(\beta E+A_h ) \right)v_{t}^{(k)} ,v_{t}^{(k)} \right)+\frac{1}{4} \beta \left(  v^{(k+1)}+v^{(k)}, v^{(k+1)}+v^{(k)} \right) \nonumber\\
...
Този параграф е посветен на техниките използвани при пресмятането на дискретната енергия в \rf{en_norm} от Консервативната схема и числените резултати получени за нея. Формула \rf{en_norm} е еквивалентна на
\begin{align}
&E_h(v^{(k)})=\nonumber\\
&\left( \left( \beta (E-\Delta_{h,2}^{-1})- \frac{\tau^2}{4}(\beta E-\Delta_{h,2} ) \right)v_{t}^{(k)} ,v_{t}^{(k)} \right)+\frac{1}{4} \beta \left(  v^{(k+1)}+v^{(k)}, v^{(k+1)}+v^{(k)} \right) \nonumber\\
&\frac{1}{4}  \left( \Delta_{h,2}(v^{(k+1)}+v^{(k)}), v^{(k+1)}+v^{(k)} \right)
- \frac{\alpha \beta}{3} \left( ((v^{(k+1)})^3,1)+((v^{(k)})^3,1) \right).
\end{align}
Разкривайки произведенията в скобите и вземайки предвид диференчното уравнение \rf{PsnDiscret} и дефинициите в \rf{assign} се получава:
\begin{align}
&E_h(v^{(k)})=\nonumber\\
&\beta \Vert v_{t}^{(k)} \Vert_{\kappa} - \beta \left( \Delta_{h,2}^{-1}  v_{t}^{(k)} ,v_{t}^{(k)} \right) +
\frac{\tau^2}{4}\left( \Delta_{h,2,y} v_{t}^{(k)} + v_{t}^{(k)} \Delta_{h,2,x}^T ,v_{t}^{(k)} \right) - \nonumber\\
& - \frac{\beta\tau^2}{4} ( v_{t}^{(k)},  v_{t}^{(k)} ) + \frac{1}{4} \beta \left(  v^{(k+1)}+v^{(k)}, v^{(k+1)}+v^{(k)} \right) \nonumber\\
&\frac{1}{4}  \left( \Delta_{h,2,y}(v^{(k+1)}+v^{(k)}) + (v^{(k+1)}+v^{(k)}) \Delta_{h,2,x}^T, v^{(k+1)}+v^{(k)} \right) \nonumber\\
& - \frac{\alpha \beta}{3} \left( ((v^{(k+1)})^3,1)+((v^{(k)})^3,1) \right).
\end{align}
След като използваме техниката за обръщане на оператора на Лаплас с ниска алгоритмична сложност и формули \rf{fpp4} и \rf{substInv}
може да се изведе следния явен израз за енергията
\begin{align}\label{DscreteEn}
&E_h(v^{(k)})=\nonumber\\
&\beta \left( v_{t}^{(k)} ,v_{t}^{(k)} \right) - \beta \left( ( S_y^T ) ^{-1} Q_h^{(k)} S_x^{-1} ,v_{t}^{(k)} \right) +
\frac{\tau^2}{4}\left( \Delta_{h,2,y} v_{t}^{(k)} + v_{t}^{(k)} \Delta_{h,2,x}^T ,v_{t}^{(k)} \right) - \nonumber\\
& - \frac{\beta\tau^2}{4} ( v_{t}^{(k)},  v_{t}^{(k)} ) + \frac{1}{4} \beta \left(  v^{(k+1)}+v^{(k)}, v^{(k+1)}+v^{(k)} \right) + \nonumber\\
&+ \frac{1}{4}  \left( \Delta_{h,2,y}(v^{(k+1)}+v^{(k)}) + (v^{(k+1)}+v^{(k)}) \Delta_{h,2,x}^T, v^{(k+1)}+v^{(k)} \right) - \nonumber\\
& - \frac{\alpha \beta}{3} \left( ((v^{(k+1)})^3,1)+((v^{(k)})^3,1) \right),
\end{align}
където $Q_h^{(k)}$ е матрица с елементи $-b_{i,j}^{(k)}/(\lambda_i + \lambda_j)$, а $-b_{i,j}^{(k)}$ са елементите на $( S_y^T )  v_{t}^{(k)} S_x$.
...
\par
1.2.1) собствените стойности $D_x$, $D_y$ и вектори $S_x$, $S_y$ съответно на матриците $-\Delta_{h,p,x}$ и $-\Delta_{h,p,y}$, 
\par
...
\par
2.1.1) $B_a:= S_y^T * ( \Delta_{h,p,y} * nonlin(s) + nonlin(s) * \Delta_{h,p,x}^T ) * S_x$
\par
...
&\frac{1}{\beta h^2} \left (  \Delta_{h,p,y} * \frac{\partial^{s-2}}{\partial t^{s-2}} V^{(k)} + \frac{\partial^{s-2}}{\partial t^{s-2}} V^{(k)} * \Delta_{h,p,x}^T  \right) 
\end{align}
\par
