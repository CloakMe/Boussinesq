...
$$
където с $\tau = T/N_t$ сме означили стъпката по времето. Стойността на неизвестната функция $u$ в точка от мрежата $(x_i,y_j,t_k)$ е означена с $u_{i,j}^k$,т.е. долния индекс се използва за пространствената дискретизация, а горния за времева дискретизация. Използвайки мрежата $\Omega_h$ може да дефинираме следните диференчни оператори:
$$ v_{\bar{x}x} = \frac{1}{h^2}(v_{i+1,j} - 2v_{i,j} + v_{i-1,j}), i = 1,\cdots, N_x-1, j = 0, \cdots, N_y $$ 
$$ v_{x} =  \frac{1}{h}(v_{i+1,j} - v_{i,j}), i = 0,\cdots, N_x-1, j = 0, \cdots, N_y$$
$$ v_{\bar{x}} =  \frac{1}{h}(v_{i,j} - v_{i-1,j}), i = 1,\cdots, N_x, j = 0, \cdots, N_y$$

$$ v_{\bar{y}y} = \frac{1}{h^2}(v_{i,j+1} - 2v_{i,j} + v_{i,j-1}), i = 0,\cdots, N_x, j = 1, \cdots, N_y-1 $$ 
$$ v_{y} =  \frac{1}{h}(v_{i,j+1} - v_{i,j}), i = 0,\cdots, N_x, j = 0, \cdots, N_y-1$$
$$ v_{\bar{y}} =  \frac{1}{h}(v_{i,j} - v_{i-1,j}), i = 0,\cdots, N_x, j = 1, \cdots, N_y$$

$$ v_{\bar{t}t} = \frac{1}{\tau^2}(v^{k+1} - 2x^{k} + v^{k-1}), k = 1,\cdots, N_{\tau}-1 $$ 
$$ v_{t} =  \frac{1}{\tau}(v^{k+1} - v^{k}), k = 0,\cdots, N_{\tau}-1 $$
$$ v_{\bar{t}} =  \frac{1}{\tau}(v^{k} - v^{k-1}), k = 1,\cdots, N_{\tau}.$$

\section{Дискретни закони за запазване на енергията и масата при непрекъснатата задача}
В пространството от функции, за които  $f_b(x) = 0$ и $\Delta f_b(x) = 0$ по границата $\partial\Omega$ т.е.
$$ \{ f_b : \Omega \rightarrow R | f_b(x) = 0, \Delta f_b(x) = 0, x \in \partial\Omega\},$$
дефинираме дискретния оператор  $A_h \in \RR^{(N_x-2) (N_y-2) \times (N_x-2) (N_y-2)}$ във вътрешността на мрежата за $(x,y) \in \Omega_h \setminus \partial\Omega$, така че да удовлетворява $A_h v=-\Delta_h v=-v_{\bar{x}x} - v_{\bar{y}y}$. Основно се използват централни крайни разлики от тип кръст, където дължината и ширината на кръста са с 3, 5 и 7 точки съответно при различните апроксимации на вторите производни $O(h^2)$, $O(h^4)$ и $O(h^6)$. По и близо до границата се използват други шаблони възползвайки се от свойствата на функционалното пространство. Матрицата $h^2A_h$ е блок-диагонална и при $O(h^2)$ изглежда така
\[
\begin{bmatrix}
    A_d           & A_1            &     0           &   \dots         & 0   \\
    A_1           & A_d            & A_1           &   0               & \vdots    \\
...
\end{bmatrix}
\]
където $A_d, A_1 \in \RR^{N_x-2 \times N_x-2}$. Матрицата $A_1=:Diag(-1)$ е диагонална матрица с $-1$ по главния диагонал. Матрицата $A_d$ е три-лентова матрица и изглежда така:
...
\end{bmatrix}
\]
В този случай се възползваме само от условието че $f_b(x) = 0$.  При $O(h^4)$ се получава следното:

\[
\begin{bmatrix}
    \bar{A}_d        	 & \bar{A}_1            	& \bar{A}_2	&    \bar{A}_3  &    0              &  \dots         & 0   \\
    A_1          		& A_d            	& A_1		&  A_2           &   0               & \dots   	 & 0 \\
    A_2          		& A_1            	& A_d		&  A_1           &   A_2            &      .           &\vdots    \\
        0           		& \ddots        	&    \ddots    &   \ddots       &     \ddots      &  \ddots     &    0 \\
\\
    0      		 &  \dots           	 &   0     	& A_2 	 & A_1 	 & A_d 	 &  A_1\\
    0              		 & \dots          	&  0               &\bar{A}_3  	 &  \bar{A}_2 & \bar{A}_1 & \bar{A}_d\\
\end{bmatrix}
\]
където $A_d, A_1, A_2,\bar{A}_d,\bar{A}_1, \bar{A}_2, \bar{A}_3 \in \RR^{N_x-2 \times N_x-2}$. Матриците $A_1=Diag(\frac{4}{3})$,  $A_2=Diag(-\frac{1}{12})$, $\bar{A}_1=Diag(\frac{29}{20})$, $\bar{A}_2=Diag(-\frac{2}{15})$, $\bar{A}_3=Diag(\frac{1}{120})$ са диагонални. Матрицата $A_d$ е пет-лентова матрица и изглежда така:

\[
\begin{bmatrix}
    -\frac{103}{15}		& \frac{29}{20}            	& -\frac{2}{15}	&    \frac{1}{120}  &    0              &  \dots         & 0   \\
    \frac{4}{3}    	&-\frac{5}{1}            	& \frac{4}{3}	&  -\frac{1}{12}    &   0               & \dots   	 & 0 \\
    -\frac{1}{12}		& \frac{4}{3}            	& -\frac{5}{1}	&  \frac{4}{3}     &   -\frac{1}{12}  &      .           &\vdots    \\
        0           		& \ddots        	&    \ddots    &   \ddots       &     \ddots      &  \ddots     &    0 \\
\\
    0      		 &  \dots           	 &   0     	& -\frac{1}{12} 	 & \frac{4}{3} 	 & -\frac{5}{1} 	 &  \frac{4}{3}\\
    0              		 & \dots          	&  0               &\frac{1}{120} 	 &  -\frac{2}{15} & \frac{29}{20} & -\frac{103}{15}\\
\end{bmatrix}
\]

Матрицата $\bar{A}_d$ е пет-лентова матрица и изглежда така:

\[
\begin{bmatrix}
    -\frac{76}{15}		& \frac{29}{20}            	& -\frac{2}{15}	&    \frac{1}{120}  &    0              &  \dots         & 0   \\
    \frac{4}{3}    	&-\frac{103}{15}            	& \frac{4}{3}	&  -\frac{1}{12}    &   0               & \dots   	 & 0 \\
    -\frac{1}{12}		& \frac{4}{3}            	& -\frac{103}{15}	&  \frac{4}{3}     &   -\frac{1}{12}  &      .           &\vdots    \\
        0           		& \ddots        	&    \ddots    &   \ddots       &     \ddots      &  \ddots     &    0 \\
\\
    0      		 &  \dots           	 &   0     	& -\frac{1}{12} 	 & \frac{4}{3} 	 & -\frac{103}{15} 	 &  \frac{4}{3}\\
    0              		 & \dots          	&  0               &\frac{1}{120} 	 &  -\frac{2}{15} & \frac{29}{20} & -\frac{76}{15}\\
\end{bmatrix}
\]
%
%\[
%\begin{bmatrix}
%    \bar{A}_d & \bar{A}_1 & \bar{A}_2 & \bar{A}_3 & \bar{A}_4 &    0            &    0     & \dots  & 0 \\
%                         \bar\bar{A}_d & \bar\bar{A}_1 & \bar{A}_2 & \bar{A}_3 & \bar{A}_4 &    0     & \dots  & 0 \\
...
%\end{bmatrix}
%\]
 Нека да разгледаме следната диференчната схема

\be\label{FDS1}
\beta (E+A_h)v_{\bar{t}t}^k +\beta A_hv^k+A_h^2 v^k -\alpha \beta A\left(\frac{(v^{k+1})^3-(v^{k-1})^3}{3(v^{k+1}-v^{k-1})} \right)=0
\ee
...
\end{align*}
След реорганизация на първия и втория член получаваме
\begin{align*}
&\left( \beta (E+A_h^{-1})- \frac{\tau^2}{2}(\beta E+A_h ) \right)v_{\bar{t}t}^k  + \frac{1}{2} (\beta E +A_h )(v^{k+1}+v^{k-1}) \\
&-\alpha \beta \frac{(v^{k+1})^3-(v^{k-1})^3}{3(v^{k+1}-v^{k-1})} =0.
\end{align*}
...
\begin{align*}
&\left( \beta (E+A_h^{-1})- \frac{\tau^2}{2}(\beta E+A_h ) \right) \left( (v^{k+1})^2 - 2v^{k+1}v^{k} + 2v^{k}v^{k-1} - (v^{k-1})^2   \right )  + \\
...
\begin{align*}
&\left( \beta (E+A_h^{-1})- \frac{\tau^2}{4}(\beta E+A_h ) \right) \left( (v^{k+1})^2 - 2v^{k+1}v^{k} + 2v^{k}v^{k-1} - (v^{k-1})^2   \right )  + \\
...
Добавя се $(v^{k})^2 - (v^{k})^2 = 0$ в десните множители на първия и втория член и $(v^{k})^3 - (v^{k})^3 = 0$ в третия, за да се получи:
 \begin{align*}
&\left( \beta (E+A_h^{-1})- \frac{\tau^2}{4}(\beta E+A_h ) \right) \left( (v^{k+1} - v^{k} )^2 - (v^{k} - v^{k-1})^2   \right )  + \\
