...
\end{align}
After rearranging system \rf{SubSys5}, a total number of $n$ linear systems are produced which are five diagonal. For system \rf{SubSys}, there are $n$ linear systems which are seven diagonal. To solve equation \rf{DLeq} for the unknown derivative $D^{\tilde s}u_h$, the right side $f_h$ is described by
$$
f_h = \Delta_h D^{\tilde s - 2} ( \alpha \beta u_h^2  + (\beta -1)u_h).
$$
