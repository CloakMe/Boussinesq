...
The paper is organized in the following way.  Taylor series expansion method for solving hyperbolic problem is presented in Section \rf{TaylorA} and  a formula for boundary conditions is posed in  Section \rf{BndS}. The application of fast Poison solvers to the numerical evaluation of the operator $(E - \tilde \Delta)^{-1}$ is explained in Section  \rf{FPSsection}. Convergence of the numerical method and tests for propagation in time of one solution are reported in Section \rf{NumTests}. 
...
Test Wave $\beta = 1$ (see Figure \rf{fig:oneWaveA}). The following test shows single solution wave with $\beta = 1$, $c = 0.9$, $\tau = 0.02$, $h = 0.1$ and $\Omega_h \times T_{\tau}$ $= [40, 40] \times [40, 80] \times [0, 40]$. This is the finest grid for this set with the highest approximation order $O(h^6 + \tau^6)$. Notice that the time domain is increased four times. Here the wave travels a distance which is $36 = 40*0.9$ units along the $y$ axis. Thus, it is obtained that $\Vert u_h(40)_{y = y*-36} - u_h(0) \Vert_{L_2} = 0.000812$ and $\Vert u_h(40)_{y = y*-36} - u_h(0) \Vert_{L_\infty} = 0.000039$.
...
We propose Taylor Series expansion method to solve BE with accuracy of second, fourth and sixth order in space and time. The advantage of this method is how easily one could calculate such complex functions. Often, the solution blows up or disperse and Figures \rf{fig:oneWaveA} and \rf{fig:oneWaveB} show that the accuracy of TS is good enough to produce stable soliton.
