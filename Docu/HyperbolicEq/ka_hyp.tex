...
\begin{abstract}
This paper evaluates propagating wave solutions to the two dimensional Boussinesq Equation (BE). To solve this nonlinear  hyperbolic problem we use  high order finite difference schemes for the spatial derivatives. Taylor Series (TS) expansion method about time is used to move the solution forward. An explicit formula for boundary Conditions (BC) is applied on the computational boundary. The resulting numerical method has high accuracy of   second, fourth and sixth order both in space and time.   The performed numerical tests exhibit good convergence and confirm the validity of the TS method.
...
In order to obtain an initial condition (IC) for equation \rf{eq1}, \rf{eq11}, another elliptic differential equation for a stationary, travelling in $y$  direction with velocity $c$ solution $U$ given by $u(x,y,t)=U(x,y-ct)$,   has to be solved. 
 The corresponding elliptic problem was treated in \cite{Ch2011} using  the perturbation solution method.  In this way   IC to \rf{eq1}, \rf{eq11}  with small wave velocities $c$ were developed.
Other methods for obtaining IC were given in \cite{chr-chr},\cite{chr-chr-07} and \cite{EllipticProblem}. 
Using these  IC with small wave speeds $c \approx 0.3$, the hyperbolic problem \rf{eq1}  was solved and the results are  reported in \cite{cher} and \cite{dani}. 
%The articles \cite{cher} and \cite{dani} showed results for the hyperbolic problem \rf{eq1} with small wave speeds $c \approx 0.3$ and used the Perturbation Solution,
The calculations in the above papers show, that the numerical solution of the hyperbolic problem with these IC either blows up or disperse in the form of ring wave expanding to infinity  after some period in time.

%An approximation for IC with small wave velocities $c$ was developed in \cite{Ch2011} using  the perturbation solution method. 
% which is developed in \cite{Ch2011}, as IC . 
...
Since the IC are  fundamental for solving the unsteady BE (1), we obtain in \cite{EllipticProblem}  more accurate   stationary, travelling in $y$  direction with velocity $c$ solutions $U$ to (1), especially for velocities $c > 0.7$.   We found also  new asymptotic boundary conditions on $\partial \Omega$ in the article \cite{BoundaryProblem}.

%for the construction of initial data of 
 Having in mind the obtained in  \cite{EllipticProblem} and \cite{BoundaryProblem} new IC and boundary conditions, we seek in this article  accurate solutions to \rf{eq1} with IC from \cite{EllipticProblem}. 



%Thus, the following article appeared \cite{critEn}, which gives the partial answer whether the IC is going to blow-up, depending on the computation of the critical energy constant for the two-dimensional BE.  
%
%
%
%The obtained results in our previous work \cite{EllipticProblem} and \cite{BoundaryProblem} serve as initial and boundary condition for the hyperbolic equation \rf{eq1}, \rf{eq11}  here. 
...
In this section, a few numerical results are presented, which concern the verification of the TS method. 
 The BE equation \rf{eqVC} is examined for two sets of parameters $\beta$ and $c$ where $\alpha = 1$ is kept constant. For the first set with $c=0.52$ and $\beta = 3$, the solutions are computed on three nested grids in the region $[-40, 40] \times [-40, 40] \times [0, 10]$. For $O(h^2 + \tau^2)$, the nested domains contain $401 \times 401$,  $801 \times 801$, $1601 \times 1601$  grid points. For $O(h^4 + \tau^4)$ and $O(h^6 + \tau^6)$, the nested domains are the same and contain $(201 \times 401 \times 126)$,  $(401 \times 801 \times 251)$ and $(801 \times 1601 \times 501)$ grid points.  For the second set with $\beta = 1$ and $c=0.9$, the solutions are also computed on three nested  grids in the region $[-40, 40] \times [-80, 80]\times [0, 10]$. For $O(h^2 + \tau^2)$, the nested domains contain $(401 \times 801 \times 1001)$,  $(801 \times 1601 \times 2001)$ and $(1601 \times 3201 \times 4001)$ grid points. For $O(h^4 + \tau^4)$ and $O(h^6 + \tau^6)$, the nested domains are the same and contain $(201 \times 401 \times 126)$,  $(401 \times 801 \times 251)$ and $(801 \times 1601 \times 501)$ grid points. Refer to Table \rf{tab:a} for details about the space and time steps $h$ and $\tau$ used in the two series  of numerical calculations.
\begin{table}[ht]
\centering
    ...
    $\beta=3$   &0.1, 0.04   &~ 0.0000e-00 &           &~0.0000e-00      &       \\
     $O(h^6)$ &0.05, 0.02 &~ 0.0000e-00 &5.8  &~0.0000e-00 & 5.6        \\
	   \hline
			\hline 
      c=0.9    &0.2, 0.01    &             &            &           &   \\
  $\beta=1$   &0.1, 0.005     &~ 0.000000  &            &~0.000000 &   \\
  $O(h^2)$ &0.05, 0.0025   &~ 0.000000  &2  &~0.000000 & 2 \\
			\hline
      c=0.9    &0.4, 0.08    &            &            &             &    \\
      $\beta=1$  &0.2, 0.04    &~ 0.000000   &           &~0.000000  &   \\
       $O(h^4)$ &0.1, 0.02   &~ 0.000000 & 3.9    &~0.000000  & 3.9  \\
    \hline
   c=0.9       &0.4, 0.08    &            &        &                  &      \\
    $\beta=1$   &0.2, 0.04   &~ 0.0000e-00 &           &~0.0000e-00      &       \\
     $O(h^6)$ &0.1, 0.02 &~ 0.0000e-00 &5.8  &~0.0000e-00 & 5.6        \\
	   \hline
			\hline 
		\end{tabular}
...
\end{figure}
\fi
Figure \ref{fig:solMax} displays the evolution of the maximum on the finest grid with the highest approximation order $O(h^6 + \tau^6)$. That is $h=0.05$, $\tau = 0.02$ for the left picture and  $h=0.1$, $\tau = 0.02$ for the right picture. 
...
Furthermore, the complexity of the algorithm with respect to the time approximation order could be described by the following table \begin{table}[!htbp]
\centering
		\begin{tabular}{||c|c|c|c||}
			\hline
			\hline
...
Test 1 Wave. The following test shows single solution wave with $\beta = 1$, $c = 0.9$, $\tau = 0.02$, $h = 0.1$ and $\Omega_h \times T_{\tau}$ $= [40, 40] \times [40, 80] \times [0, 40]$. This is the finest grid with the highest approximation order $O(h^6 + \tau^6)$. It behaves like a soliton, i.e. preserves its shape and maximum.
...
We propose Taylor series expansion method  to solve BE with  accuracy of   second, fourth and sixth order in space and time.  The advantage of this method is ...???

We perform a series on numerical experiments to validate the numerical method. The results show that the approximate Taylor series method achieves the prescribed high accuracy. 
...
% for wave speeds near the upper limit $c_{max} = 1/\sqrt{\beta}$, $c < c_{max}$, 


 The use of regular grid with equidistant step size $h$ turns out to be good choice to test head-on collision of the obtained two waves. Furthermore, it provides valuable information with sufficient point count on the computational boundary. 

%The performed numerical tests exhibit good convergence and solitonic-like behavior for larger times $t_n \in T_{\tau}$ which validates the TS method. 
