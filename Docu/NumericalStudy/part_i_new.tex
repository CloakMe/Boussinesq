...
A polynomial zero order fit is done at each $k^{th}$ successful iteration $k \in N$ for $x_{i}U(x_{i},0,k\tau)$, $x_{i} \in [x_{m}, x_{end}]$ and $y_{i}U(0,y_{i},k\tau)$, $y_{i} \in [y_{m}, y_{end}]$ . If the fits $c_{x}$ and $c_{y}$
\begin{equation}\label{stopCrit}
|c_{x}^{k\tau} - c_{x}^{(k+1)\tau}| < \epsilon \quad \textrm{and} \quad |c_{y}^{k\tau} - c_{y}^{(k+1)\tau}| < \epsilon
\end{equation}
cease to change significantly in time then the solution has converged.
