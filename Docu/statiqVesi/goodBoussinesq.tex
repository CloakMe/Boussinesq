...
Table \ref{tableD} is analogous to Table \ref{tableC} and measures the convergence speed based on the results of the discrete energy \rf{energy1}. The only difference is that in the last Table \ref{tableD}, the energy is calculated over four nested meshes and no exact formula is used in the Runge's rule. The convergence rate of both solution and energy is found to be in correspondence with the second spatial order approximation that is used.
...
The mass is compared on four different grids with space and time steps defined in Table \rf{tableMass} - second column. The first column shows the method which is used - Conservative FDS or Taylor method. For the cases with 
$p=2$, we use Trapezoidal rule for integration. In case of $p=4$ and Taylor method, we use Simpson (1/3) rule. The results, with the initial mass and its maximum deviation, are displayed in Table \rf{tableMass}, in the third and fourth columns respectively.  
%D
\begin{table}[ht]
\centering
\small
