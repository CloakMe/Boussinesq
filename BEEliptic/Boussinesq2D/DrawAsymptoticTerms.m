function DrawAsymptoticTerms(x,y,h,al,bt,c,theta,bigU,bigUTimeDer,bigIC,U,compBox,secondDerivative)
    ...
    lenx = floor(length( x(zeroX:end) )/4*3);
...
%     figure(10)
%     mesh(xx,yy, (res(zeroX + lenx:end-barF,zeroY - leny:zeroY + leny) + res1(zeroX + lenx:end-barF,zeroY - leny:zeroY + leny) )');
%     title('bt * ((1-c^2) * U_y_y + U_x_x) - al * bt * Delta(U^2) (no boundary values)');
